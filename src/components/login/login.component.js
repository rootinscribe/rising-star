...
import { addCurrentPerson } from '../../redux/modules/session'
import { addOrderCurrentPersonId, clearOrder } from '../../redux/modules/order'
...
class Login extends React.Component {
  ...
  login = async (userAccount) => {
    try {
      ...
      if (response.status === 200) {
        ...
        this.clearOrder()
        this.updateCurrentPersonId(credentials.id, credentials.accessKey, credentials.secretKey, credentials.region)
        this.updateOrderCurrentPersonId(credentials.id)
  ...
  updateCurrentPersonId = (id, accessKey, secretKey, region) => {
    const { addCurrentPerson } = this.props
    addCurrentPerson(id, accessKey, secretKey, region)
  }
  ...
  updateOrderCurrentPersonId = (id) => {
    const { addOrderCurrentPersonId } = this.props
    addOrderCurrentPersonId(id)
  }
      ...
      <View style={styles.centeredItems}>
        ...
        <View style={[styles.loginForm, styles.centeredItems]}>
          ...
          <Text style={styles.accountText}>
            ...
            <Text style={styles.createAccount} onPress={() => navigation.navigate('signUp')}> CLICK HERE</Text>
          </Text>
        </View>
      </View>
    )
